...
\begin{itemize}
    \item To design and implement a dual-verifiable federated learning framework where both clients and the server provide ZKPs.
    \item To implement zk-SNARK-based proofs for verifying both client-side local training (PySNARK) and server-side aggregation (Groth16).
    \item To introduce a dynamic proof adjustment mechanism with three rigor levels that modifies proof complexity based on training stability.
    \item To ensure the framework is efficient and deployable with minimal communication overhead.
    \item To validate the system using comprehensive multi-dataset evaluation demonstrating consistent performance.
    \item To demonstrate practical viability while providing cryptographic security guarantees.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{experiments/comprehensive_real_analysis.png}
\caption{Multi-Dataset Analysis}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Rigor} & \textbf{Gen Time} & \textbf{Verify Time} & \textbf{Circuit Size} & \textbf{Memory} \\
\midrule
High & 2.58s ± 0.21s & 89ms & 2.1M constraints & 847 MB \\
Medium & 1.12s ± 0.15s & 45ms & 890k constraints & 356 MB \\
Low & 0.43s ± 0.09s & 23ms & 180k constraints & 127 MB \\
