...
class TestBasicFLWorkflow:
    ...
    def test_single_round_training(self, client_datasets, server_config, client_config):
        ...
        strategy = create_server_strategy(
            model_fn=model_fn,
            enable_zkp=False,  # Disable ZKP for basic test
            **server_config,
        )
        ...
        for client in clients:
            # Simulate fit call
            result = client._simulate_fit(initial_params, local_epochs=1)
        ...
        aggregated_params = strategy.aggregate_fit(
            server_round=1, results=list(zip(clients, client_results)), failures=[]
        )

        # Verify results
        assert aggregated_params is not None
        assert len(aggregated_params.parameters) == len(initial_params)

        # Parameters should have changed
        for init, final in zip(
            initial_params, parameters_to_ndarrays(aggregated_params.parameters)
        ):
    ...
    def test_multi_round_training(self, client_datasets, server_config, client_config):
        ...
        # Create server strategy
        strategy = create_server_strategy(
            model_fn=model_fn, enable_zkp=False, num_rounds=3, **server_config
        )
        ...
        for round_num in range(1, 4):
            ...
            client_results = []
            for client in clients:
                result = client._simulate_fit(current_params, local_epochs=1)
                client_results.append(result)
            ...
            current_params = parameters_to_ndarrays(aggregated.parameters)
    ...
    def test_client_dropout_handling(
        ...
        strategy = create_server_strategy(
            model_fn=model_fn,
            enable_zkp=False,
            min_fit_clients=2,  # Allow training with fewer clients
            **server_config,
        )
        ...
        assert len(aggregated.parameters) == len(initial_params)
    ...
    def test_heterogeneous_data_sizes(self, server_config, client_config):
        ...
        strategy = create_server_strategy(
            model_fn=model_fn, enable_zkp=False, **server_config
        )
...
class TestAggregationIntegration:
    ...
    def test_fedjscm_integration(self, client_datasets, server_config, client_config):
        ...
        assert "server_momentum" in momentum_state
...
class TestErrorHandling:
    ...
    def test_invalid_client_updates(
        ...
        result1 = clients[0]._simulate_fit(initial_params, local_epochs=1)
    ...
    def test_insufficient_clients(self, client_datasets, server_config, client_config):
        ...
        strategy = create_server_strategy(
            model_fn=model_fn, enable_zkp=False, **strict_config
        )
    ...
    def test_nan_parameter_handling(
        ...
        try:
            ...
            if aggregated is not None:
                final_params = parameters_to_ndarrays(aggregated.parameters)
...
class TestPerformanceIntegration:
    ...
    def test_large_model_training(self, server_config, client_config):
        ...
        client_results = []
        for client in clients:
            result = client._simulate_fit(initial_params, local_epochs=2)
            client_results.append(result)
    ...
    def test_memory_efficiency(self, client_datasets, server_config, client_config):
        ...
        for round_num in range(1, 6):
            ...
            initial_params = parameters_to_ndarrays(aggregated.parameters)
...
class TestConfigurationIntegration:
    ...
    def test_various_aggregation_configs(
        ...
        for momentum in momentum_values:
            strategy = create_server_strategy(
                model_fn=model_fn, enable_zkp=False, momentum=momentum, **server_config
            )
    ...
    def test_various_client_configs(self, client_datasets, server_config):
        ...
        for local_epochs in local_epochs_configs:
            ...
            client_results = []
            for client in clients:
                result = client._simulate_fit(initial_params, local_epochs=local_epochs)
                client_results.append(result)
